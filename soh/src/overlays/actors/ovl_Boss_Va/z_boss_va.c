        ...
        if (effect->type != VA_NONE) {
            ...
            if (effect->type == VA_BLOOD) {
                if (effect->mode < BLOOD_SPOT) {
                    ...
                    floorY = BgCheck_EntityRaycastDown1(&play->colCtx, &sp90, &sp94);
            ...
            if (effect->type == VA_GORE) {
                if (effect->mode == GORE_PERMANENT) {
                    ...
                    effect->rot.x += 0x1770;
                    floorY = BgCheck_EntityRaycastDown1(&play->colCtx, &sp7C, &sp80);
...
void BossVa_SpawnTumor(PlayState* play, BossVaEffect* effect, BossVa* this, Vec3f* offset, s16 scale,
    ...
    for (i = 0; i < ARRAY_COUNT(sVaEffects); i++, effect++) {
        if (effect->type == VA_NONE) {
            ...
            if (((i % 4) == 0) || (mode == 2)) {
                Audio_PlaySfxGeneral(NA_SE_EN_BALINADE_BREAK, &effect->pos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            }
            break;
        }
    }
