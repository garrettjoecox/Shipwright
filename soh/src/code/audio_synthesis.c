...
void func_800DB03C(s32 arg0) {
    ...
    baseIndex = gAudioCtx.numNotes * arg0;
...
void func_800DB2C0(s32 updateIndexStart, s32 noteIndex) {
    ...
    for (i = updateIndexStart + 1; i < gAudioCtx.audioBufferParameters.updatesPerFrame; i++) {
        temp_v1 = &gAudioCtx.noteSubsEu[(gAudioCtx.numNotes * i) + noteIndex];
...
Acmd* AudioSynth_DoOneAudioUpdate(s16* aiBuf, s32 aiBufLen, Acmd* cmd, s32 updateIndex) {
    ...
    while (i < count) {
        cmd = AudioSynth_ProcessNote(noteIndices[i], &gAudioCtx.noteSubsEu[t + noteIndices[i]],
                                     &gAudioCtx.notes[noteIndices[i]].synthesisState, aiBuf, aiBufLen, cmd,
                                     updateIndex);
        i++;
    }
    ...
    } else 
    {
        ...
        for (curPart = 0; curPart < nParts; curPart++) {
            ...
            if (audioFontSample->codec == CODEC_ADPCM || audioFontSample->codec == CODEC_SMALL_ADPCM) {
                if (gAudioCtx.curLoadedBook != audioFontSample->book->book) {
                    u32 nEntries;
                    switch (bookOffset) {
                        case 1:
                        ...
                        case 3:
                        default:
                            gAudioCtx.curLoadedBook = audioFontSample->book->book;
                            break;
                    }
