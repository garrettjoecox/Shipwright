...
class TextFactory : public ResourceFactory
{
  public:
    ...
    std::shared_ptr<IResource> ReadResourceXML(std::shared_ptr<ResourceInitData> initData, tinyxml2::XMLElement *reader) override;
    void WriteResourceXML(std::shared_ptr<IResource> resource, tinyxml2::XMLDocument* writer) override;
};
...
class TextFactoryV0 : public ResourceVersionFactory
{
  public:
    ...
    void WriteFileXML(tinyxml2::XMLDocument* writer, std::shared_ptr<IResource> resource) override;
};
}; // namespace LUS
