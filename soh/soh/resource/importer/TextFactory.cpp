...
void TextFactory::WriteResourceXML(std::shared_ptr<IResource> resource, tinyxml2::XMLDocument* writer) {
    std::shared_ptr<ResourceVersionFactory> factory = nullptr;
    ...
    if (factory == nullptr) {
        SPDLOG_ERROR("Failed to write Text with version {}", resource->GetInitData()->ResourceVersion);
        return;
    }

    factory->WriteFileXML(writer, resource);
}
        ...
        while (textNode != nullptr) {
            ...
            if (textNodeName == "TextNode") {
                if (textNode->GetText() == nullptr) {
                    SPDLOG_WARN("TextNode has no text. TextEntry:{} Text Resource:{}", entry.id, textResource->GetInitData()->Path);
...
void TextFactoryV0::WriteFileXML(tinyxml2::XMLDocument* writer, std::shared_ptr<IResource> resource) {
    std::shared_ptr<Text> textResource = std::static_pointer_cast<Text>(resource);
